...
class CodeNodeC(Node):
    def __init__(self, code, description, value, parent, varcontrol, valuecontrol, valueFormat = 'none', valueType = 'none'):
        super(CodeNodeC, self).__init__(code, description, parent)
        self.value = value
        self.default = value
        self.typeInfo = 'CODE'
        self.valueFormat = valueFormat
    ...
    def _recurseXml(self, parent):
        child = ET.SubElement(parent, 
                           ...
                           attrib = {'code': self.code,
                                     ...
                                     'valuecontrol' :'%s' % self.valuecontrol})
        ET.SubElement(child, 
                   tag = 'VALUE', 
                   attrib = {'valeur': '%f' % self.value})
...
class XmlReader(object):
    def __init__(self, paramFile):
        self._date = datetime.strptime("2011-01-02","%Y-%m-%d").date()
    ...
    def handleNodeList(self, nodeList, parent):
        ...
        for element in nodeList:
            if element.nodeType is not element.TEXT_NODE and element.nodeType is not element.COMMENT_NODE:
                ...
                elif element.tagName == "MULTIPLE":
                    code = element.getAttribute('code')
                    desc = element.getAttribute('description')
                    valueFormat = element.getAttribute('format')
                    valueType = element.getAttribute('type')
                    varcontrol = self.handleVarControl(element.getElementsByTagName("VARCONTROL")[0], self._date)
                    print varcontrol
                    if not varcontrol is None :
                        seuils, values = multiple_lists(element, date)
                        node = MultipleNode(code, desc, parent, varcontrol, seuils, values, valueType, valueFormat)
                    else :
                        val = self.handleValues(element, date)
                        node = CodeNode(code, desc, float(val), parent, valueFormat, valueType)
                        
#                     for decla in element.getElementsByTagName("DECLA"):
#                         val, valcontrol = self.handleControl(decla, self._date)
#                         print val, valcontrol
#                         if not val is None:
#                             node = CodeNodeC(code, desc, float(val), parent, valueFormat, valueType, varcontrol, valcontrol) 
...
#                         print "OH",  val
#                         valcontrol = self.handleValuesControl(gen, self._date)
#                         print valcontrol
#                         if not val is None:
#                             node = CodeNodeC(code, desc, float(val), parent, valueFormat, valueType, varcontrol, valcontrol) 
    ...
    def handleVarControl(self, element, date):
        # TODO gérer les assiettes en mettan l'assiette à 1 si elle n'existe pas
    ...
    def handleControl(self, element, date):
        # TODO gérer les assiettes en mettan l'assiette à 1 si elle n'existe pas
        for val in element.getElementsByTagName("VALUE"):
            try:
                deb = datetime.strptime(val.getAttribute('deb'),"%Y-%m-%d").date()
                fin   = datetime.strptime(val.getAttribute('fin'),"%Y-%m-%d").date()
                if deb <= date <= fin:
                    return float(val.getAttribute('valeur')), datetime.strptime(val.getAttribute('valeurcontrol'),"%Y-%m-%d").date()
            except Exception, e:
                code = element.getAttribute('code')
                raise Exception("Problem error when dealing with %s : \n %s" %(code,e))
        return None
